...
export function getPageLink(store, linkName) {
  if (Array.isArray(store)) {
    const arr = store.filter((el) => el.link === linkName);
    return arr[0].link;
  } else {
    alert('Incorrect store type, should be arr. Message from Helper');
  }
}
